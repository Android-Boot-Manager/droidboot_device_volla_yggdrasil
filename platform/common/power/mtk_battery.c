...
{
	int offset, val;
	offset = fdt_node_offset_by_compatible(g_fdt, -1, "mediatek,bat_gm30");
	if (offset >= 0) {
		val = chr_fdt_getprop_u32(g_fdt, offset, "DISABLE_MTKBATTERY");
		if (val)
			fg_cust_data.disable_mtkbattery = val;

		val = chr_fdt_getprop_u32(g_fdt, offset, "CAR_TUNE_VALUE");
		if (val)
			fg_cust_data.car_tune_value = val * UNIT_TRANS_10;

		val = chr_fdt_getprop_u32(g_fdt, offset, "FG_METER_RESISTANCE");
		if (val)
			fg_cust_data.fg_meter_resistance = val;

		val = chr_fdt_getprop_u32(g_fdt, offset, "R_FG_VALUE");
		if (val)
			fg_cust_data.r_fg_value = val * UNIT_TRANS_10;

		val = chr_fdt_getprop_u32(g_fdt, offset, "PMIC_MIN_VOL");
		if (val)
			fg_cust_data.pmic_min_vol = val;

		val = chr_fdt_getprop_u32(g_fdt, offset, "POWERON_SYSTEM_IBOOT");
		if (val)
