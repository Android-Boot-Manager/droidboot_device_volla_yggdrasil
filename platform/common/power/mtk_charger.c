...
#endif
//prize-add-pengzhipeng-20191104-end
...
//prize modified by huarui, eta6937 support, 20190305-start
	#if defined(HL7005ALL_CHARGER_SUPPORT)
		ret = mtk_charger_enable_charging(primary_mchr, true);	//charge enable will load default reg val
		if (ret < 0)
		...
		if (ret < 0)
			dprintf(CRITICAL, "%s: enable charging failed, ret = %d\n", __func__, ret);
	#endif
//prize modified by huarui, eta6937 support, 20190305-end
