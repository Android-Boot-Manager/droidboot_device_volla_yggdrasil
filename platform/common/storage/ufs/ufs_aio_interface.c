...
__WEAK status_t interface_ufs_erase(u64 address, u64 length)
{
	int ret;
	struct ufs_hba *hba = ufs_aio_get_host(UFS_DEFAULT_HOST_ID);
...
__WEAK status_t interface_get_ufs_info(struct ufs_info_struct* info)
{
	...
	if (hba->dev_info.product_id[0] != 0)
		strlcpy((char *)info->cid, hba->dev_info.product_id, MAX_PRODUCT_ID_LEN);
	LOGD("[UFS] product_id: %s\n", info->cid);

	if (hba->dev_info.product_revision_level[0] != 0)
		strlcpy((char *)info->fwver, hba->dev_info.product_revision_level, MAX_PRODUCT_REVISION_LEVEL_LEN);
	UFS_DBG_LOGD("[UFS] fwver: %s\n", info->cid);
