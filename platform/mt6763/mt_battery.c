...
#if defined(HL7005ALL_CHARGER_SUPPORT)
	hl7005all_hw_init();
	hl7005all_charging_enable(bEnable);
	hl7005all_dump_register();
#endif
//prize added by huarui, hl7005 support, 20190111-end
...
void mt65xx_bat_init(void)
{
	...
	chr_volt = get_charger_volt(1);
	if (chr_volt > V_CHARGER_MAX) {
		dprintf(CRITICAL, "Charger Over Voltage:%d  V_CHARGER_MAX=%d\n, power off...", chr_volt,V_CHARGER_MAX);
		mt6575_power_off();
	}
#endif
