...
//prize added by lipengpeng, cw2015 support, 20191224-start
#if defined(MTK_CW2015_SUPPORT)
#include <platform/cw2015_fuel_gauge.h>
#endif
//prize added by lipengpeng, cw2015 support, 20191224-end
...
#if defined(PRIZE_WIRELESS_RECEIVER_MAXIC_MT5715)
...
//prize added by huarui, cw2015 support, 20190111-start
	...
	{	
		...
		if (ret < 0)
		{
			dprintf(CRITICAL, "%s, cw2015 init failed(%d)\n", __FUNCTION__,ret);
		}	
	}

	batcap = cw2015_get_capacity();
	if (batcap < 0 || batcap > 100)
		dprintf(CRITICAL, "cw2015 get cap failed.(%d)\n",batcap);
		
	batvolt = cw2015_get_voltage();
	if (batvolt <= 0){
		dprintf(CRITICAL, "cw2015 get vol failed.(%d)\n",batvolt);
		//batvolt = get_i_sense_volt(1); //if failed and get pmic's Vbat
	}
#endif
	/*prize--low power to alert users when booting up hjian 20171027*/
...
//prize added by huarui, cw2015 support, 20190111-start
	#if defined(MTK_CW2015_SUPPORT)
		val = batvolt; //prize--low power to alert users when booting up hjian 20171027
	#else
...
//prize added by huarui, cw2015 support, 20190111-start
#if defined(MTK_CW2015_SUPPORT)
	dprintf(CRITICAL, "cw2015 ===batvolt= %d, batcap=%d\n", val,batcap);
//prize add by lipengpeng 20190920 start
	if ((val < BATTERY_LOWVOL_THRESOLD) || (batcap < 1)) {
//prize add by lipengpeng 20190920 end
#else
...
//prize added by huarui, cw2015 support, 20190111-start
	...
	dprintf(CRITICAL, "cw2015 get batvolt=(%d) batcap=%d\n",batvolt,batcap);
#endif
	/*prize--low power to alert users when booting up hjian 20171027*/
//prize added by huarui, cw2015 support, 20190111-end
