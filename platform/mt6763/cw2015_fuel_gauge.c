...
extern void *g_fdt;
static kal_uint16 cw2015_i2c_id = I2C0;
...
int cw2015_init(void)
{
	...
	//get i2c_id from dts
	offset = fdt_node_offset_by_compatible(g_fdt,0,"cellwise,cw2015");
	if (offset < 0) {
		dprintf(CRITICAL, "[cw2015] Failed to find cellwise,cw2015 in dtb\n");
	}else{
		//batinfo
		data = fdt_getprop(g_fdt, offset, "batinfo", &len);
		if (len > 0 && data){
			dprintf(INFO, "[cw2015] batinfo len(%d)\n",len);
			if (len != SIZE_BATINFO){
				dprintf(INFO, "[cw2015] get bat batinfo fail len(%d) != SIZE_BATINFO(%d)\n",len,SIZE_BATINFO);
			}else{
				memcpy(config_info,data,len);
				dprintf(INFO,"[cw2015]batinfo ");
				for (i=0;i<SIZE_BATINFO;i++){
					printf("%x ",config_info[i]);
				}
				printf("\n");
			}
		}
		//i2c id
		offset = fdt_parent_offset(g_fdt,offset);
		data = fdt_getprop(g_fdt, offset, "id", &len);
		if (len > 0 && data){
			cw2015_i2c_id = fdt32_to_cpu(*(unsigned int *)data);
	...
	dprintf(CRITICAL, "cw2015 lk init ok\n");
    return 0;
}
