...
#else 
printk("ATA check lk size = 0x%x,0x%x,0x%x,0x%x\n",x0_MSB,x0_LSB,x1_MSB,x1_LSB); 
#endif 
...
printk("ATA read buf new lk size = 0x%x,0x%x,0x%x,0x%x,ret= %d ret1= %d\n",read_buf1[0],read_buf1[1],read_buf1[2],read_buf1[3],ret,ret1); 
...
static unsigned int lcm_compare_id(void)
{
	...
	return (LCM_ID_ILI9881C == id)?1:0;//0x0098810c=ili9881c-0d
	//return 1;
}
...
LCM_DRIVER ili9881c_hd_dsi_vdo_cmi_ata_lcm_drv = {
