...
//prize added by huarui, lcm power control, 20190121-start
#if defined(USE_DTB_NO_DWS)
#include <libfdt.h>
extern void *g_fdt;
...
static int gpio_tpd_rst_pin = -1;
static int gpio_lcd_bias_enp_pin = -1;
static int gpio_lcd_bias_enn_pin = -1;
static int i2c_bias_id = -1;
static int i2c_bias_addr = -1;
static int i2c_bias_exist = 0;
static char init_lcm_ldo_gpio_form_dtb(){
void *lk_drv_fdt = get_lk_overlayed_dtb();//prize huarui
	...
	offset = fdt_node_offset_by_compatible(lk_drv_fdt,0,"ti,tps65132");
	if (offset < 0){
		dprintf(CRITICAL, "[lcm_pmic] Failed to find ti,tps65132 in dtb\n");
	}else{
		data = fdt_getprop(lk_drv_fdt, offset, "reg", &len);
		if (len > 0 && data){
			i2c_bias_addr = fdt32_to_cpu(*(unsigned int *)data);
		}
		dprintf(INFO, "[lcm_pmic] i2c reg(%x)\n",i2c_bias_addr);
		
		offset = fdt_parent_offset(lk_drv_fdt,offset);
		data = fdt_getprop(lk_drv_fdt, offset, "id", &len);
		if (len > 0 && data){
			i2c_bias_id = fdt32_to_cpu(*(unsigned int *)data);
		}
		dprintf(INFO, "[lcm_pmic] i2c id(%d)\n",i2c_bias_id);
		
		if ((i2c_bias_id >=0 )&&(i2c_bias_addr >= 0)){
			i2c_bias_exist = 1;
		}
	}
	
	offset = fdt_node_offset_by_compatible(lk_drv_fdt,0,"prize,lcm_power_gpio");
	if (offset < 0) {
		dprintf(CRITICAL, "[lcm_pmic] Failed to find prize,lcm_power_gpio in dtb\n");
		return -1;
	}else{
		data = fdt_getprop(lk_drv_fdt, offset, "gpio_lcd_ldo18_pin", &len);
		if (len > 0 && data){
			gpio_lcd_ldo18_pin = fdt32_to_cpu(*(unsigned int *)data);
			dprintf(INFO, "[lcm_pmic] gpio_lcd_ldo18_pin(%d)\n",gpio_lcd_ldo18_pin);
		}
		data = fdt_getprop(lk_drv_fdt, offset, "gpio_lcd_ldo28_pin", &len);
		if (len > 0 && data){
			gpio_lcd_ldo28_pin = fdt32_to_cpu(*(unsigned int *)data);
			dprintf(INFO, "[lcm_pmic] gpio_lcd_ldo28_pin(%d)\n",gpio_lcd_ldo28_pin);
		}
		data = fdt_getprop(lk_drv_fdt, offset, "gpio_tpd_rst_pin", &len);
		if (len > 0 && data){
			gpio_tpd_rst_pin = fdt32_to_cpu(*(unsigned int *)data);
			dprintf(INFO, "[lcm_pmic] gpio_tpd_rst_pin(%d)\n",gpio_tpd_rst_pin);
		}
		data = fdt_getprop(lk_drv_fdt, offset, "gpio_lcd_bias_enp_pin", &len);
		if (len > 0 && data){
			gpio_lcd_bias_enp_pin = fdt32_to_cpu(*(unsigned int *)data);
			dprintf(INFO, "[lcm_pmic] gpio_lcd_bias_enp_pin(%d)\n",gpio_lcd_bias_enp_pin);
		}
		data = fdt_getprop(lk_drv_fdt, offset, "gpio_lcd_bias_enn_pin", &len);
		if (len > 0 && data){
...
#endif
//prize added by huarui, lcm power control, 20190121-end
...
int display_bias_enable(void)
{
	...
	display_bias_vneg_set(5500);//as in kernel,default 5500mV
	return ret;
}
...
static int lcm_i2c_read_byte (kal_uint8 addr, kal_uint8 *dataBuffer)
{
	kal_uint32 ret = I2C_OK;
	kal_uint16 len;
	struct mt_i2c_t PMU_i2c;
	*dataBuffer = addr;
	...
	PMU_i2c.id = i2c_bias_id;
	PMU_i2c.addr = i2c_bias_addr;
	PMU_i2c.mode = ST_MODE;
	PMU_i2c.speed = 100;
	len = 1;
...
static int lcm_i2c_write_byte(kal_uint8 addr, kal_uint8 value)
{
	kal_uint32 ret_code = I2C_OK;
	kal_uint8 write_data[2];
	kal_uint16 len;
...
static int lcm_i2c_reg_mask(kal_uint8 addr, kal_uint8 val, kal_uint8 mask)
{
	kal_uint8 PMU_reg = 0;
	kal_uint32 ret = 0;
	...
	ret = lcm_i2c_read_byte(addr, &PMU_reg);

	PMU_reg &= ~mask;
	PMU_reg |= val;

	ret = lcm_i2c_write_byte(addr, PMU_reg);
...
int display_bias_vpos_set(int mv)
{
	...
	if (mv > 6000){
		mv = 6000;
		dprintf(CRITICAL, "[lcm_pmic] set vpos %d exceed maximum, set to 6\n",mv);
	}
	if (mv < 4000){
		mv = 4000;
		dprintf(CRITICAL, "[lcm_pmic] set vpos %d lower than minimum, set to 4\n",mv);
	}
	mv = (mv-4000)/100;
    
	/* set AVDD 5.4v, (4v+28*0.05v) */
	ret = lcm_i2c_reg_mask(0x00, mv, (0x1F << 0));
	if (ret != I2C_OK){
		dprintf(CRITICAL, "[lcm_pmic]cmd=%0x--i2c write error %d\n", 0x00,ret);
	}
...
int display_bias_vneg_set(int mv)
{
	...
	/* set AVEE 5.4v, (4v+28*0.05v) */
	ret = lcm_i2c_reg_mask(0x01, mv, (0x1F << 0));
	if (ret != I2C_OK){
		dprintf(CRITICAL, "[lcm_pmic]cmd=%0x--i2c write error %d\n", 0x01,ret);
	}
...
{
	int ret = 0;
#if defined(GPIO_TPD_RST_PIN)
	if (enable){
		mt_set_gpio_mode(GPIO_TPD_RST_PIN, GPIO_MODE_00);//define in dws
		mt_set_gpio_dir(GPIO_TPD_RST_PIN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_TPD_RST_PIN, GPIO_OUT_ONE);
	}else{
		mt_set_gpio_mode(GPIO_TPD_RST_PIN, GPIO_MODE_00);//define in dws
		mt_set_gpio_dir(GPIO_TPD_RST_PIN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_TPD_RST_PIN, GPIO_OUT_ZERO);
	}
#elif defined(USE_DTB_NO_DWS)
	if (!is_init_lcm_ldo_gpio){
		init_lcm_ldo_gpio_form_dtb();
	}
	if (gpio_tpd_rst_pin >= 0){
		if (enable){
			mt_set_gpio_mode(0x80000000 | gpio_tpd_rst_pin, GPIO_MODE_00);
			mt_set_gpio_dir(0x80000000 | gpio_tpd_rst_pin, GPIO_DIR_OUT);
			mt_set_gpio_out(0x80000000 | gpio_tpd_rst_pin, GPIO_OUT_ONE);
		}else{
			mt_set_gpio_mode(0x80000000 | gpio_tpd_rst_pin, GPIO_MODE_00);
			mt_set_gpio_dir(0x80000000 | gpio_tpd_rst_pin, GPIO_DIR_OUT);
			mt_set_gpio_out(0x80000000 | gpio_tpd_rst_pin, GPIO_OUT_ZERO);
		}
	}
#endif
	return ret;
